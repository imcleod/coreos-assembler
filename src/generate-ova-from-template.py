...
vmdk_xml_filename = os.path.join(vmdk_working_path, "disk.ovf")
vmdk_xml_file = open(vmdk_xml_filename, "w")
vmdk_xml_file.write(vmdk_xml)
vmdk_xml_file.close()

tar = tarfile.open(output_image, 'w')
tar.add(vmdk_xml_filename, arcname="disk.ovf")
tar.add(vmdk_image, arcname="image.vmdk")
tar.close()
